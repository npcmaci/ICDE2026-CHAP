...
class CASTLE(object):
    def __init__(self, num_train, lr  = None, batch_size = 32, num_inputs = 1, num_outputs = 1,
        ...
        self.supervised_loss = tf.reduce_mean(tf.reduce_sum(tf.square(self.out_layer['nn_' + str(self.num_inputs - 1)] - self.y),axis=1),axis=0)
    ...
    def fit(self, X, y,num_nodes, X_val, y_val, X_test, y_test):
        ...
        for step in range(1, self.max_steps):
            ...
            print("Step " + str(step) + ", Loss= " + "{:.4f}".format(loss)," h_value:", h_value, f", Epoch time: {epoch_time:.4f}s")
    ...
    def pred(self, X):
        return self.sess.run(self.out_layer['nn_' + str(self.num_inputs - 1)], feed_dict={self.X: X, self.keep_prob:1, self.is_train : False, self.noise:0})
